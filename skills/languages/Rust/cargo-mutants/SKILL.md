...
Distinguish them before reaching for `exclude_re`. The default is to **kill the mutant by writing a stronger test**, not to exclude it. Reach for `exclude_re` only when you can articulate why no test can distinguish the mutation — make that articulation with the **Equivalence soundness** checks, not by assertion.
...
The criteria above define *what* equivalence is; this is the decision procedure that separates a **true equivalent** (no test can distinguish the mutant from the original) from a **coverage gap** (a test could distinguish it, none does yet). Run it before adding any equivalence `exclude_re`. A mutant sitting in `missed.txt` plus a plausible equivalence narrative is **not** sufficient to exclude — a coverage gap produces exactly that pair. This is a different axis from anchor validity: the **Verification protocol** and **Pre-run anchor validation** confirm a regex hits the intended mutant, whereas this confirms the claim that the mutant is unkillable is actually true. A pattern can be perfectly anchored and still exclude a killable mutant.
...
1. **Over-exclusion cross-check (mechanical).** The removed set — the mutants an `exclude_re` pattern strips — must contain nothing the test suite kills. Compute it as a set difference, the way **Pre-run anchor validation** diffs to see what a pattern removes: list the stripped universe `U` (`cargo mutants --list --config <stripped-toml>`, plus `--workspace` per that section), list it again with the candidate pattern applied as `K` (`cargo mutants --list --config <stripped-toml> --exclude-re='<pattern>'`), and take `removed_set = U \ K` (e.g. `comm -23` over the two sorted listings). `K` is the *kept* set — the universe minus the pattern's targets — so do **not** intersect `K` itself with `caught.txt`; `K` is non-empty for any healthy config and would falsely read as a swarm of over-exclusions. Intersect `removed_set` with `caught.txt`; the intersection must be empty. Any excluded mutant that appears in `caught.txt` is killed by some test, hence not equivalent — an over-exclusion, usually an over-broad function-name anchor sweeping in killable siblings. Two provenance requirements make the intersection sound:
   - `caught.txt` must come from a run that **actually tested** every mutant in `removed_set`, at the **same source revision and workspace/package scope** as `U`. Use a **stripped-config run** — all `exclude_re` entries emptied, the same config that produced `U` — so no pre-existing exclude masks a candidate target. A plain "latest full run" is sound only when the live config carries no other `exclude_re` overlapping `removed_set`: a candidate target also covered by a pre-existing exclude was never tested in that run, so it sits in `removed_set` yet cannot appear in `caught.txt` — exactly the false pass this check guards against. Likewise never use a run with the candidate exclude already applied, nor a narrower-scoped run (`--file`, a single `--package`): there too the mutant is absent from both `caught.txt` and `missed.txt` because it was never tested.
