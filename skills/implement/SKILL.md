...
4. **Textual-surface drift sweep.** Renames and module-structure changes have to be threaded through every secondary textual surface, not just the primary identifier. After each rename / removal in the diff, run `rg <old-identifier>` over the touched crate(s) and resolve every remaining hit: panic / `expect` / `assert!` messages, error format strings, inline comments, doctest blocks, rustdoc links, error-variant `detail` strings, README / module-level prose. After each `mod` / `pub mod` add / remove / rename, re-read the parent module's `//!` docstring against the current set of children — claims like "currently exposes X" or "lands in a subsequent phase" rot the moment Y is added. For each new identifier whose name *asserts* a property (`random_right_canonical_mps_*`, `is_normalized_*`, `into_canonical_*`), confirm the implementation actually provides that property; helpers that wrap a parametrized API call should be named after the parameter values they pin, not after the operational role they happen to serve in the calling code.
