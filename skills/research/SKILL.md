...
Run `gh-body-check` on the plan body with artifact kind `issue` before any `gh-post` invocation. Any ⚠ blocks posting until the iteration `gh-body-check` step 4 states clears it. If that changed the plan, re-show it for approval before posting — Step 6 approved the text as it stood.
