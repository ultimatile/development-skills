...
Run `gh-body-check` on the plan body before any `gh-post` invocation. Resolve any ⚠, re-discharge the plan's evidence claims, and re-run the check until every ⚠ is closed in the sense `finding-triage`'s **Closure** defines. If that changed the plan, re-show it for approval before posting — Step 6 approved the text as it stood.
