...
**New-comment claim sweep (author-blindspot mitigation).** Authors read intent; reviewers read literal text. For every comment / docstring **newly added or modified** in the diff, cross-check the literal claim against the code (catches `rel_tol = 1e-9` in comment vs `1e-4` in code; `SU(2)` in comment vs `U(2)` from the generator; `V_g^T` in comment vs `V_g^†` in code; `simultaneously diagonalizes` without a tolerance qualifier). This sweep also covers **fenced code-block examples in newly added / modified prose docs** (`README`, `CONTRIBUTING.md`, `docs/`): an illustrative snippet that presents real workspace symbols must name symbols that actually exist (catches `HeffError` in a `#[source]` example when the real type is `DmrgHeffError`).
