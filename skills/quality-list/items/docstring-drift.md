...
For a shift in a symbol's side effects or result identity (a memoization change), consumer code affected by the shift — call-site reliance on the effect, observers of the mutated state surface — is `impact-verification`'s concern; this section owns the documentation surface.
