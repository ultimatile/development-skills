...
5. **Settle the root**, on `diff-root`'s terms: the branch this work merges into, as its remote-tracking ref. The ref step 3 cut the branch from is not it — a local branch is routinely ahead of or behind what the work will merge into, and commits inherited that way must stay in every gate's scope because they land through the same PR. Take a root the caller supplied; with none, ask. Record it beside the branch, and state it: Phase 2 passes it on, and a caller continuing past this skill needs it for its own gates.
