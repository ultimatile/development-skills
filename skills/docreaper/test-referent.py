...
def expect(name, line, relation, contains=(), not_contains=(), end_line=None,
           referent_start=None):
    ...
    def checker(rec):
        ...
        if ok and referent_start is not None:
            ok = b["referent"] is not None and \
                b["referent"]["span"]["start_line"] == referent_start
...
def rust_bad_attribute(rec):
    b = block_at(rec, 1)
    check("rust: malformed attribute in the composite referent -> parse-error",
          b and b["unreached"] == "parse-error", json.dumps(b))


def one_block(name, end_line):
    def checker(rec):
        check(name, len(rec["blocks"]) == 1
...
def julia_comment_between(rec):
    b = block_at(rec, 2)
    check("julia: comment between string and function -> string is no docstring, "
          "comment binds to fn",
          all(x["relation"] != "docstring" for x in rec["blocks"])
          and b and b["relation"] == "next-sibling", json.dumps(rec["blocks"]))


def python_shebang_pragma(rec):
    lines = [e["line"] for e in rec["excluded_lines"]]
    texts = [e["text"] for e in rec["excluded_lines"]]
    check("python: shebang and coding pragma are excluded lines, not blocks",
          lines == [1, 2] and texts[0].startswith("#!")
          and block_at(rec, 1) is None and block_at(rec, 2) is None,
          json.dumps(rec))
...
def c_error_locality(rec):
    near = block_at(rec, 5)
    far = block_at(rec, 1)
    check("c: block near the missing semicolon is parse-error, distant block binds",
          near and near["unreached"] == "parse-error"
          and far and far["unreached"] is None, json.dumps([far, near]))


def json_structural(rec):
    b = block_at(rec, 1)
    check("output: comment text with quotes and backslashes survives JSON",
          b is not None and '"quoted"' in b["text"] and "\\" in b["text"],
          json.dumps(b))


def comments_only(rec):
    check("comments-only file: blocks bind to the file root",
          rec["unreached"] is None and len(rec["blocks"]) == 2
          and all(b["relation"] == "header" for b in rec["blocks"]),
          json.dumps(rec))


def empty_file(rec):
    check("empty file: reached, no blocks",
          rec["unreached"] is None and rec["blocks"] == [], json.dumps(rec))


SPECIMENS = [
    ("rs", "/// Adds one.\nfn f(x: u32) -> u32 { x + 1 }\n",
     expect("rust: tight /// binds to fn", 1, "next-sibling",
            contains=["fn f"], not_contains=["x + 1"])),
    ("rs", "/// dangling doc\n\nfn f() {}\n",
     expect("rust: /// + blank line is a header", 1, "header")),
    ("rs", "/// Adds one\n/// to the input.\nfn f(x: u32) -> u32 { x + 1 }\n",
     expect("rust: /// run merges and binds to fn", 1, "next-sibling",
            contains=["fn f"], end_line=2, referent_start=3)),
    ("rs", "/// a\n\n/// b\nfn f() {}\n",
     expect_many("rust: blank line splits a /// run",
                 [(1, "header", []), (3, "next-sibling", [])])),
    ("rs", "fn g() {}\n/// eof doc",
     expect("rust: /// at EOF without newline is a header (no false trailing)",
            2, "header")),
    ("rs", "//! module doc\n\nfn f() {}\n",
     expect("rust: //! binds to the file, comments stripped from the referent",
            1, "inner-doc", contains=["fn f"], not_contains=["module doc"])),
    ("rs", "/*! module doc */\n\nfn f() {}\n",
     expect("rust: /*! */ binds to the file", 1, "inner-doc")),
    ("rs", "//! module doc\n/// fn doc\nfn f() {}\n",
     expect_many("rust: //! + /// stack splits into two blocks with distinct referents",
                 [(1, "inner-doc", []), (2, "next-sibling", [])])),
    ("rs", "// SPDX-License-Identifier: MIT\n// Copyright example\n/// fn doc\nfn f() {}\n",
     expect_many("rust: bare license run splits from /// run; both bind to fn",
                 [(1, "next-sibling", ["fn f"]), (3, "next-sibling", ["fn f"])])),
    ("rs", "/// doc\n#[inline]\nfn f() {}\n",
     expect("rust: /// above #[inline] reaches fn with the attribute in the referent",
            1, "next-sibling", contains=["#[inline]", "fn f"])),
    ("rs", "/// doc\n#[inline]\n#[must_use]\nfn f() -> u32 { 1 }\n",
     expect("rust: /// above two attributes reaches fn with both in the referent",
            1, "next-sibling", contains=["#[inline]", "#[must_use]", "fn f"])),
    ("rs", "/// doc\n#[inline(]\nfn f() {}\n", rust_bad_attribute),
    ("rs", "/* Reset the device.\n\n   Caller holds the lock. */\nfn f() {}\n",
     one_block("rust: /* .. blank .. */ is one block", 3)),
    ("rs", "fn f() -> u32 {\n    // picks the key\n    let key = { 1 + 2 };\n    key\n}\n",
     expect("rust: a let binding's block-expression initializer stays in the referent",
            2, "next-sibling", contains=["1 + 2"])),
    ("jl", '"""Sorts the rows."""\nfunction rank(rows)\n    sort(rows)\nend\n',
     expect("julia: docstring above function", 1, "docstring",
            contains=["function rank"], not_contains=["sort("])),
    ("jl", '"doc"\nmodule M\nend\n',
    ...
    ("jl", '"doc"\nf(x) = x + 1\n',
     expect("julia: docstring above short-form def", 1, "docstring", contains=["f(x)"])),
    ("jl", 'function f()\n    "hello"\nend\n',
     no_docstring_block("julia: a string as a function's return value is not a block",
                        absent_text="hello")),
    ("jl", '@doc "Adds one." g(x) = x + 1\n',
     expect("julia: @doc two-argument form binds the string to the target",
            1, "docstring", contains=["g(x)"])),
    ("jl", '@doc "Adds one."\nh(x) = x + 1\n',
     expect("julia: @doc single-argument form binds to the next sibling",
            1, "docstring", contains=["h(x)"])),
    ("jl", '"dangling"\n\nf(x) = x\n',
     no_docstring_block("julia: docstring + blank line does not attach and is not a block",
                        absent_text="dangling")),
    ("jl", '"not a doc"\n# comment\nf(x) = x\n', julia_comment_between),
    ("jl", 'md"text"\nf(x) = x\n',
     no_docstring_block("julia: prefixed string literal is not a docstring")),
    ("jl", 'function f(x)\n    x + 1\nend  # end of f\n',
     expect("julia: trailing comment after end", 3, "trailing",
            contains=["function f"])),
    ("jl", 'y = begin\n    a = 1\n    a + 1\n    # last item\nend\n',
     expect("julia: comment as last item of a begin block is no false trailing",
            4, "header")),
    ("jl", 'begin\n"doc"\nf(x) = x\nend\n',
    ...
    ("jl", '"""Doc."""\nfunction f(x)\n    x + 1\n    # inner note\nend\n',
     expect("julia: a comment after the stripped body is stripped from the referent",
            1, "docstring", contains=["function f"], not_contains=["inner note"])),
    ("py", '"""Module doc."""\nx = 1\n',
     expect("python: module docstring binds to the file", 1, "docstring")),
    ("py", '# leading comment\n"""Module doc."""\nx = 1\n',
     expect("python: a leading comment does not stop the module docstring "
            "(comments are not statements)", 2, "docstring")),
    ("py", 'def f(x):\n    """Doc."""\n    return x\n',
     expect("python: def docstring binds to the declaration without the body",
            2, "docstring", contains=["def f"], not_contains=["return"])),
    ("lua", "-- Doubles the input.\nfunction f(x)\n  return x * 2\nend\n",
     expect("lua: a -- comment above a function binds to it", 1, "next-sibling",
            contains=["function f"])),
    ("py", 'class C:\n    """Doc."""\n    pass\n',
     expect("python: class docstring", 2, "docstring", contains=["class C"])),
    ("py", '@deco\ndef f(x):\n    """Doc."""\n    return x\n',
     expect("python: docstring inside a decorated def", 3, "docstring",
            contains=["def f"], not_contains=["return"])),
    ("py", '# doc comment\n@deco\ndef f(x):\n    return x\n',
     expect("python: comment above a decorator -> decorator and def line, body stripped",
            1, "next-sibling", contains=["@deco", "def f"], not_contains=["return"])),
    ("py", 'x = 1\n"not a docstring"\ny = 2\n',
     no_docstring_block("python: a non-first string is not a docstring")),
    ("py", 'MAX = 5  # attempts before giving up\nTIMEOUT = 30\n',
     expect("python: trailing comment binds to its own line", 1, "trailing",
            contains=["MAX"], not_contains=["TIMEOUT"])),
    ...
    ("py", 'x = 1\n# coding: utf-8\ny = 2\n',
     expect("python: a line-2 cookie after code on line 1 is not a pragma",
            2, "next-sibling", contains=["y = 2"])),
    ("py", 'x = 1  # coding: utf-8\ny = 2\n',
    ...
    ("c", "int x;\nvoid g(void) { f(host, /*port=*/443, 3); }\n",
     expect("c: /*port=*/ as a middle argument binds forward to 443", 2, "forward",
            contains=["443"], not_contains=["host"])),
    ("c", "int x;\nvoid g(void) { f(/*port=*/443, 3); }\n",
     expect("c: /*port=*/ as the first argument binds forward to 443", 2, "forward",
            contains=["443"])),
    ("c", "struct S {\n    int retries;  ///< handshake attempts\n    int timeout;\n};\n",
     expect("c: ///< binds backward to its own line's declaration", 2, "trailing",
            contains=["retries"], not_contains=["timeout"])),
    ("cpp", "/// Adds one.\nint f(int x) { return x + 1; }\n",
     expect("cpp: a doc comment above a function binds without the body",
            1, "next-sibling", contains=["int f"], not_contains=["return"])),
    ("cpp", "int a = 1; // note on a\nint b = 2;\n",
     expect("cpp: trailing comment", 1, "trailing", contains=["int a"],
            not_contains=["int b"])),
    ("c", "int a = 1; // note on a\nint b = 2;\n",
     expect("c: same-row trailing wins over the adjacent next declaration",
            1, "trailing", contains=["int a"], not_contains=["int b"])),
    ("sh", "cat > out.conf <<'EOF'\n# workers\nworkers = 4\nEOF\n",
     no_docstring_block("bash: comment-shaped lines inside a heredoc are not blocks",
                        absent_text="workers")),
    ("sh", "# checks the guard\nrun_scan missing.md >/dev/null 2>&1\n",
     expect("bash: a redirected command keeps its command words in the referent",
            1, "next-sibling", contains=["run_scan", "/dev/null"])),
    ("sh", "x=1  # trailing note\ny=2\n",
     expect("bash: trailing comment", 1, "trailing")),
    ("js", "/** Sorts. */\nexport function sortRows(rows) { return rows.sort(); }\n",
     expect("js: an exported function's doc strips the body like a local one",
            1, "next-sibling", contains=["export function sortRows"],
            not_contains=["rows.sort"])),
    ("py", '@deprecated("use parse_config")\ndef f(x):\n    \'\'\'Deprecated: use parse_config.\'\'\'\n    return x\n',
     expect("python: a decorated def's docstring referent includes the decorator",
            3, "docstring", contains=["@deprecated", "def f"], not_contains=["return"])),
    ("js", "/** Sorts the rows. */\nconst rank = (rows) => [...rows].sort();\n",
     expect("js: an arrow-function binding carries its whole text as referent",
            1, "next-sibling", contains=["sort"])),
    ("go", "package p\n\n// Rank sorts the rows.\nfunc Rank(x int) int { return x }\n",
     expect("go: a bare // run adjacent to a declaration binds to it",
            3, "next-sibling", contains=["func Rank"])),
    ("go", "// separated header\n\npackage p\n",
    ...
    ("rb", "# Doubles the input.\ndef f(x)\n  x * 2\nend\n",
     expect("ruby: a # run above a method binds to it without the body",
            1, "next-sibling", contains=["def f"], not_contains=["x * 2"])),
    ("rb", "x = 1  # trailing note\ny = 2\n",
     expect("ruby: trailing comment", 1, "trailing")),
    ("java", "class C {\n    /** Adds one. */\n    int f(int x) { return x + 1; }\n}\n",
     expect("java: Javadoc above a method binds to it without the body",
            2, "next-sibling", contains=["int f"], not_contains=["return"])),
    ("java", "/** A container. */\nclass C {\n    int f(int x) { return x + 1; }\n}\n",
     expect("java: a class doc's referent strips the class body",
...
def exit_code_contract(tmp):
    ...
    check("cli: a missing path -> exit 2 naming the path",
...
def main():
    with tempfile.TemporaryDirectory() as tmp:
        ...
        for i, (ext, source, _) in enumerate(SPECIMENS):
            ...
            paths.append(path)
        records = run(paths)
        check("output: one JSON Lines record per input file, in order",
              len(records) == len(paths)
              and all(r["path"] == p for r, p in zip(records, paths)))
        check("output: referent/relation nullity tracks parse-error on every block",
