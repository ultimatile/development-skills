...
- **Lane 3 — subagent.** Spawn a fresh-context subagent. Give it the full gate scope, matching what `/code-review` sees: the branch's committed diff against its base (`git diff @{upstream}...HEAD`, falling back to `main...HEAD`), plus uncommitted changes (`git diff HEAD`), plus the contents of untracked files (list them with `git ls-files --others --exclude-standard`, then read each). Instruct it to review adversarially for bugs, contract drift, and quality issues at the caller's effort (`medium` = standard pass; `high` = exhaustive per-hunk pass), and to return findings as `file:line — description — severity` or an explicit clean verdict.

## Exhaustion

If every lane is abandoned without a valid review, **halt and surface the state to the user** — never proceed toward a commit with an unreviewed diff. Only the user may waive the gate; a waived gate is a skipped gate.

## Telemetry notes

At gate time, note the lane used for each iteration (e.g. `lanes: [1, 1, 2]`) so the post-run `review-telemetry` record can carry it in the gate's `config`; a user-waived gate is recorded per `review-telemetry`'s skipped-gate rule.
