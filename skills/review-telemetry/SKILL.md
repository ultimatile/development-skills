...
- `findings[].disposition` uses the `finding-triage` SSOT slugs verbatim; read the set from that file rather than from a copy here. `waive` was added to that catalogue after this log began; schema 4 marks the boundary, per the compatibility rule above.
