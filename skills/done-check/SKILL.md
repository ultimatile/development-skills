...
5. Close every concern before proceeding, on `finding-triage`'s **Closure** terms — which disposition closes, and who may close it, is stated there. State concretely what will change. Exactly one case reaches no disposition at all: step 4's row whose auditor quoted no clause even after its re-dispatch. That one closes by the user's decision to proceed with it unjudged, which waives this gate for that row rather than disposing of the finding.
