...
5. Close every concern before proceeding, on `finding-triage`'s **Closure** terms — which disposition closes, and who may close it, is stated there. A row left holding a transient disposition cannot close; resolve it, or — for a row **Closure** cannot close — let the user waive it with a stated reason. That waiver closes no disposition; it is this gate's own, and **Closure** does not reach it. The row stays ⚠ with the waiver recorded in its Note.
