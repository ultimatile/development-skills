...
End-to-end wrapper.
Runs `research-and-implement-quaere` then `review-pipeline` back to back.
...
1. **Phase A** — invoke `research-and-implement-quaere $ARGUMENTS`.
   Branch baseline, research, and implementation are handled there.
   **Skip `/implement-quaere`'s terminal `/done-check`** — Phase B owns the audit.
...
Do not auto-merge.
Plan approval inside `/research-quaere` is also unchanged.
