...
- **waive** — the finding is valid and its fix is understood and local, this run is not to make it, and the user directs that **nothing be carried past this run**: no follow-up issue is filed. Only the user may waive, and only where making the fix would damage something worth more than the defect. The waiver carries the reasoning, which states that cost and, where the finding is `critical`, why shipping the specified behavior wrong on the happy path is acceptable. The finding is closed once that reasoning is recorded with this run's other dispositions.
