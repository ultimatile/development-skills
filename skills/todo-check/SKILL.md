...
- **`impact-verification`** — Build the impact list now, before editing. Trace public-symbol callers if no formal list exists. If a unit will make a value class newly reachable or unreachable in a symbol's returns (with or without a signature change), or make a same-signature shift of its element ordering or evaluation timing — plan the consumer trace now (the item body defines the classes, the diff-visible evidence forms, and the trace bound: direct consumers plus one pass-through hop).
