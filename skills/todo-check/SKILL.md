...
- **`escape-hatch-necessity`** — If a unit will introduce a static-guarantee escape hatch (Rust `unsafe`, C++ `reinterpret_cast` / `const_cast`, TS `any` / `as`), plan to derive its necessity first: confirm no safe construct (trait / interface dispatch, generics, enum) keeps the type concrete at the use site. Treat the hatch as a last resort behind that derivation, not a default for bridging a generic to a concrete type.
