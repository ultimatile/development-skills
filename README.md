...
| `docreaper` | Audit comments and docstrings for prose the code they annotate already carries, identifying each comment's referent with tree-sitter (run via `uv`; the first run fetches grammars). Use when a codebase has accumulated comments that restate their own referent, duplicate each other, or lean on a change history the file does not hold |
